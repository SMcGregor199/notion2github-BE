...
- Resource Guide URL enrichment has a separate signed webhook and uses `RESOURCE_GUIDE_WEBHOOK_VERIFICATION_TOKEN`; its optional `RESOURCE_GUIDE_TEXT_MODEL` falls back to `CMS_TEXT_MODEL`, then `gpt-5.6-luna`. It reuses the existing private `OPENAI_API_KEY`, `NOTION_API_KEY`, and `NOTION_RESOURCES_DATABASE_ID` values.
