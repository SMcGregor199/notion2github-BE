...
import { maybeUpdateRSSForBlogRefresh } from "../src/rss/autoRSSUpdate.ts";

async function fetchAndStoreLatestData(){
    const store = getStore("content", { consistency: "strong" });
    const previousManifest = await store.get("content/manifest.json", {type:"json"});
    ...
    await maybeUpdateRSSForBlogRefresh(blogData, {
        contentHash: hashKey,
        previousContentHash: previousManifest?.current_key,
    });
