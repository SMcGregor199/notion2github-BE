...
export function renderNewsletterEmail(input: { intro: string; title: string; summary: string; slug: string; imageUrl: string; linkedinUrl: string }): string {
  ...
  const image = isHttpUrl(input.imageUrl)
    ? `<tr><td style="padding:0 0 28px"><img src="${escapeHtml(input.imageUrl)}" alt="" width="640" style="display:block;width:100%;max-width:640px;height:auto;border:0;border-radius:12px" /></td></tr>`
    : "";
  return `<!doctype html><html><body style="margin:0;padding:0;background:#ffffff;color:#111827;font-family:Arial,sans-serif"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff" style="width:100%;background:#ffffff"><tr><td align="center" style="padding:32px 20px"><table role="presentation" width="680" cellspacing="0" cellpadding="0" border="0" style="width:100%;max-width:680px;margin:0 auto"><tr><td style="padding:0 20px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%"><tr><td style="padding:0 0 28px;font-size:17px;line-height:1.65;white-space:pre-line">${escapeHtml(input.intro)}</td></tr>${image}<tr><td style="padding:0 0 16px;font-size:30px;line-height:1.2;font-weight:700"><a href="${escapeHtml(articleUrl)}" style="color:#111827;text-decoration:underline">${escapeHtml(input.title)}</a></td></tr><tr><td style="padding:0 0 24px;font-size:17px;line-height:1.65">${escapeHtml(input.summary)}</td></tr><tr><td style="padding:0 0 28px"><a href="${escapeHtml(articleUrl)}" style="display:inline-block;background:#d86f44;color:#ffffff;padding:12px 18px;border-radius:999px;text-decoration:none;font-weight:700">Read the article</a></td></tr><tr><td style="padding:0 0 32px;font-size:16px;line-height:1.65">Want to discuss it? <a href="${escapeHtml(input.linkedinUrl)}" style="color:#0a66c2;text-decoration:underline">Join the conversation on LinkedIn</a>.</td></tr><tr><td style="border-top:1px solid #dbe2ea;padding:24px 0 0;font-size:12px;line-height:1.5;color:#64748b">You received this because you subscribed to Notes from Shayne. <a href="{{{RESEND_UNSUBSCRIBE_URL}}}" style="color:#0a66c2;text-decoration:underline">Unsubscribe</a>.</td></tr></table></td></tr></table></td></tr></table></body></html>`;
}
