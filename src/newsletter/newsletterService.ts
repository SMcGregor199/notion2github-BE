...
async function sendConfirmationEmail(input: SubscriberInput, token: string): Promise<void> {
  ...
  await resendRequest("/emails", { method: "POST", headers: { "Idempotency-Key": `confirm-${hashToken(token)}` }, body: JSON.stringify({ from: process.env.RESEND_FROM_EMAIL || "Shayne McGregor <updates@shaynemcgregor.dev>", reply_to: process.env.RESEND_REPLY_TO || "updates@shaynemcgregor.dev", to: [input.email], subject: "Confirm your Blog Updates subscription", html: `<p>Hi ${escapeHtml(input.firstName)},</p><p>Please confirm that you want to receive Blog Updates from Shayne McGregor.</p><p><a href="${escapeHtml(url)}">Confirm subscription</a></p><p>If you did not request this, you can ignore this email.</p>` }) });
}
